...
fn execMatch(self: *Vm) anyerror!void {
    ...
    const re = (value.asRegex(right)) orelse return self.ctx.err(
        "Match right hand side must be a regex, got: {s}",
        .{value.typeOf(right)},
        error.InvalidMatch,
        offset,
    );
    ...
    if (!value.isAnyStr(left)) return self.ctx.err(
        "Match left hand side must be a string, got: {s}",
        .{value.typeOf(left)},
        error.InvalidMatch,
        offset,
    );
...
fn execCapture(self: *Vm) anyerror!void {
    ...
    const match_obj_ptr = value.asMatch(m) orelse return self.ctx.err(
        "`capture` only works on regex matches, got: {s}",
        .{value.typeOf(m)},
    ...
    if (value.asUint(arg)) |u| {
        ...
        "`capture` arg must be unsigend integer index or a string, got: {s}",
        .{value.typeOf(arg)},
        error.InvalidCapture,
        offset,
    );
...
fn execNext(self: *Vm) anyerror!void {
    ...
    const m = self.value_stack.pop();
    const match_obj_ptr = value.asMatch(m) orelse return self.ctx.err(
        "`next` only works on regex matches, got: {s}",
        .{value.typeOf(m)},
        error.InvalidNext,
        offset,
    );
...
fn execReset(self: *Vm) anyerror!void {
    ...
    const m = self.value_stack.pop();
    const match_obj_ptr = value.asMatch(m) orelse return self.ctx.err(
        "`reset` only works on regex matches, got: {s}",
        .{value.typeOf(m)},
        error.InvalidReset,
        offset,
    );
