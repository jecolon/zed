...
pub fn hash(self: Value) u64 {
    return switch (self.ty) {
        .boolean => |b| std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]bool{b})),
        .float => |f| std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]f64{f})),
        .func => |f| std.hash.Wyhash.hash(seed, f.instructions),
        .int => |i| std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]i64{i})),
        .list => |l| std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]usize{@ptrToInt(l)})),
        .map => |m| std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]usize{@ptrToInt(m)})),
        .range => |r| std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&r)),
        .string => |s| std.hash.Wyhash.hash(seed, s),
        .uint => |u| std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]u64{u})),
        else => unreachable,
    };
}
pub fn InnerType(self: Value) type {
    return switch (self.ty) {
        .boolean => bool,
        .float => f64,
        .func => Value.Function,
...
pub fn unwrap(self: Value) InnerType(self) {
    return switch (self.ty) {
        .boolean => |b| b,
        .float => |f| f,
        .func => |f| f,
