...
const IndexContext = struct {
    ...
    pub fn hash(self: IndexContext, idx: u32) u64 {
        return self.values.items[idx].hash();
    }
};
...
const ValueAdapter = struct {
    ...
    pub fn eql(self: ValueAdapter, a: u64, b: u32) bool {
        return a == self.values.items[b].hash();
    }

    pub fn hash(_: ValueAdapter, adapted_key: u64) u64 {
        return adapted_key;
    }
};
...
value_store: std.ArrayList(Value),
value_table: std.HashMapUnmanaged(u32, void, IndexContext, std.hash_map.default_max_load_percentage),
...
pub fn init(
    ...
    var self = Vm{
        ...
        .value_store = std.ArrayList(Value).init(allocator),
        .value_table = .{},
    };
    ...
    // Common values
    try self.value_store.append(Value{ .ty = .{ .boolean = false } }); // 0
    try self.value_store.append(Value{ .ty = .{ .boolean = true } }); // 1
    try self.value_store.append(Value{ .ty = .nil }); // 2
...
fn execBoolTrue(self: *Vm) !void {
    self.ip.* += 1;
    try self.value_stack.append(1);
    self.ip.* += 2;
}
fn execNil(self: *Vm) !void {
    self.ip.* += 1;
    try self.value_stack.append(2);
    self.ip.* += 2;
}
...
fn execFormat(self: *Vm) !void {
    ...
    try self.pushValue([]const u8, buf.items);
}
...
fn execString(self: *Vm) !void {
    ...
    var writer = buf.writer();
    var i: usize = 0;
    while (i < len) : (i += 1) _ = try writer.print("{}", .{self.popValue()});
    try self.pushValue([]const u8, buf.items);
}
...
fn execFunc(self: *Vm) !void {
    ...
    try self.pushValue(Value.Function, .{
        .name = func_name,
        .params = params,
...
fn execDefine(self: *Vm) !void {
    ...
    // Value
    const rindex = self.value_stack.items[self.value_stack.items.len - 1];
    // Define
    try self.scope_stack.store(name, rindex);
}
...
fn execStore(self: *Vm) !void {
    ...
    // Value
    const rindex = self.value_stack.items[self.value_stack.items.len - 1];
    const rvalue = self.popValue();
    // Store
    if (combo == .none) {
        try self.scope_stack.update(name, rindex);
        try self.value_stack.append(rindex);
    } else {
        const old_value = &self.value_store.items[self.scope_stack.load(name).?];
        ...
        const index = self.getOrPutValue(new_value.InnerType(), new_value.unwrap());
        try self.scope_stack.update(name, index);
        try self.value_stack.append(index);
    }
}
fn pushExisting(self: *Vm, v: Value) !void {
    const index = self.getOrPutValue(v.InnerType(), v.unwrap());
    try self.value_stack.append(index);
}
...
fn execDiv(self: *Vm) anyerror!void {
    ...
    if (left.div(right)) |quotient| {
        try self.pushExisting(quotient);
    } else |err| return self.ctx.err("{s} / {s} ?", .{ @tagName(left.ty), @tagName(right.ty) }, err, offset);
}
fn execMod(self: *Vm) anyerror!void {
    ...
    if (left.mod(right)) |remainder| {
        try self.pushExisting(remainder);
    } else |err| return self.ctx.err("{s} % {s} ?", .{ @tagName(left.ty), @tagName(right.ty) }, err, offset);
}
fn execComparison(self: *Vm, opcode: Compiler.Opcode) !void {
    ...
    try self.value_store.append(if (result) 1 else 0);
}
fn execEqNeq(self: *Vm, opcode: Compiler.Opcode) !void {
    ...
    try self.value_store.append(if (comparison) 1 else 0);
}
fn execConcat(self: *Vm) anyerror!void {
    ...
    try self.pushValue([]const u8, buf);
}
...
fn execNot(self: *Vm) !void {
    ...
    try self.value_stack.append(if (value.ty.boolean) 0 else 1);
}
fn execNeg(self: *Vm) !void {
    ...
    switch (value.ty) {
        .float => |f| try self.pushValue(f64, -f),
        .int => |i| try self.pushValue(i64, -i),
        .uint => |u| try self.pushValue(i64, -@intCast(i64, u)),
...
fn getOrPutValue(self: *Vm, comptime T: type, v: T) !u32 {
    const h = switch (T) {
        bool => std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]bool{v})),
        f64 => std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]f64{v})),
        Value.Function => std.hash.Wyhash.hash(seed, v.instructions),
        i64 => std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]i64{v})),
        *std.ArrayList(Value) => std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]usize{@ptrToInt(v)})),
        *std.StringHashMap(Value) => std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]usize{@ptrToInt(v)})),
        [2]usize => std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&v)),
        []const u8 => std.hash.Wyhash.hash(seed, v),
        u64 => std.hash.Wyhash.hash(seed, std.mem.sliceAsBytes(&[1]u64{v})),
        else => unreachable,
    };

    const value_adapter: ValueAdapter = .{ .values = &self.value_store };
    if (self.value_table.getEntryAdapted(h, value_adapter)) |entry| {
        return entry.key_ptr.*;
    } else {
        const index = @intCast(u32, self.value_store.items.len);

        try self.value_store.append(switch (T) {
            bool => Value{ .ty = .{ .boolean = v } },
            f64 => Value{ .ty = .{ .float = v } },
            Value.Function => Value{ .ty = .{ .func = v } },
            i64 => Value{ .ty = .{ .int = v } },
            *std.ArrayList(Value) => Value{ .ty = .{ .list = v } },
            *std.StringHashMap(Value) => Value{ .ty = .{ .map = v } },
            [2]usize => Value{ .ty = .{ .range = v } },
            []const u8 => Value{ .ty = .{ .string = v } },
            u64 => Value{ .ty = .{ .uint = v } },
            else => unreachable,
        });

        const index_context = .{ .values = &self.value_store };
        try self.value_table.putContext(self.allocator, index, {}, index_context);
        return index;
    }
}

fn pushValue(self: *Vm, comptime T: type, v: T) !void {
    const index = try self.getOrPutValue(T, v);
    try self.value_stack.append(index);
}
fn popValue(self: *Vm) *Value {
    const index = self.value_stack.pop();
    return &self.value_store.items[index];
}
...
test "Vm Value store" {
    ...
    const ctx = Context{ .filename = "inline", .src = "foo" };
    ...
    const three_index = try vm.getOrPutValue(u64, 3);
    const another_three_index = try vm.getOrPutValue(u64, 3);
    const four_index = try vm.getOrPutValue(i64, -4);
    const str_index = try vm.getOrPutValue([]const u8, "foo");
    try std.testing.expectEqual(@as(u32, 0), three_index);
    try std.testing.expectEqual(@as(u32, 0), another_three_index);
    try std.testing.expectEqual(@as(u32, 1), four_index);
    try std.testing.expectEqual(@as(u32, 2), str_index);
    try std.testing.expectEqualStrings("foo", vm.value_store.items[str_index].ty.string);
}
